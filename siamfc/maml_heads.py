...
from .maml_basicstructure import MetaBatchNormLayer,MetaConv2dLayer
...
class SiamFC(nn.Module):
    def __init__(self, out_scale=0.001, BN=True ,device =None, args=None):
        super(SiamFC, self).__init__()
        self.out_scale = out_scale
        self.BN = BN
        if BN:
            self.map_norm = MetaBatchNormLayer(1,device,args)
    ...
    def forward(self, z, x,num_step,params):
        if self.BN:
            if params is not None:
                ...
                return self.map_norm(self._fast_xcorr(z, x),num_step,params=params['map_norm'])
            else:
    ...
    def _fast_xcorr(self, z, x):
        ...
        nx, c, h, w = x.size()
        x = x.view(-1, nz * c, h, w)
        out = F.conv2d(x, z, groups=nz) # 應該可以不用換成meta架構
