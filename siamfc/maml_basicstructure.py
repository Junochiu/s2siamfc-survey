...
class MetaConvNormLayerReLU(nn.Module):
    def __init__(self, in_channel, out_channel, stride=1, kernel_size=3, padding=0, use_bias=True, args=None, normalization=True,
                 relu=True,meta_layer=True, no_bn_learnable_params=False, device=None, groups=1):
        """
           ...
           meta-conv etc.
           :param in_channel: C of the image input shape in the form (b, c, h, w)
        ...
        self.args = args
        self.num_filters = out_channel  # out_channels
    ...
    def build_block(self):
        """
        guess x and out was used to get initial block weight? -> since we take pretrained as initialization, command
        lines with x and out
        """

        #x = torch.zeros(self.input_shape)

        #out = x  # get input shape from here

        self.conv = MetaConv2dLayer(in_channels=self.in_channel, out_channels=self.num_filters,
                                    kernel_size=self.kernel_size,
                                    stride=self.stride, padding=self.padding, use_bias=self.use_bias,groups=self.groups)

        #out = self.conv(out)

        if self.normalization:
            if self.args.norm_layer == "batch_norm":
                self.norm_layer = MetaBatchNormLayer(self.num_filters, track_running_stats=True,
                                                     ...
                                                     args=self.args)
            #batch norm layer is used in SiamFC command LayerNorm
